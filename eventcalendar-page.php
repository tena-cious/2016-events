										...
										<div class="wpb_text_column wpb_content_element  whiteText">
											<div class="wpb_wrapper">
												<?php 
													...
													$children = get_pages( 'sort_column=menu_order&title_li=&child_of=' . $page_id . '&echo=0' );
													if ($children) {
														$childUrl = $children[0]->guid;
														$childId = $children[0]->ID;
														$childTitle = $children[0]->post_title; ?>


														<p style="text-align: center;">
															» <a href="<?php echo '?page_id=' . $childId ?>" style="text-transform: uppercase;"><?php echo $childTitle ?></a> «
														</p>
													...
													if ($query->max_num_pages > 1) { // check if the max number of pages is greater than 1  ?>
														<p style="text-align: center;">
															<?php echo get_previous_posts_link( '» MORE EVENTS «' ); // display all previous events link ?>
															<?php if (get_previous_posts_link() && get_next_posts_link()) { echo '&nbsp;&nbsp;&nbsp;&nbsp;'; } ?>
															<?php echo get_next_posts_link( '» MORE EVENTS «', $query->max_num_pages ); // display older posts link ?>
														</p>
												<?php } ?>
											</div>
										</div>
